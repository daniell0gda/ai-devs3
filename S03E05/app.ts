...
import type {PathSegment, RecordShape} from 'neo4j-driver';
...
const driver = neo4j.driver('neo4j://localhost:7687', neo4j.auth.basic('neo4j', '4vQz#KKx13cvo6$'));
...
async function main(){
  ...
  let users = await buildRelationshipPath('Rafał', 'Barbara');
  ...
  await post_response('connections', users.join(', '));
}

async function buildRelationshipPath(forUser:string, toUser:string){
  try {
    ...
    let path =  result.records[0].get('p').segments.map((segment:PathSegment)=> segment.start.properties.username) as string[];
...
async function createKnowsRelationship(u1:string, u2:string){
...
export async function centrala_api<T>(query:unknown){
  try {
    ...
    let responseData = (await response.json()).reply as T;
